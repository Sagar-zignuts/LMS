...
const createAuthor = async (req, res) => {
  ...
  try {
    const query =
      "INSERT INTO authors (name, gender, profile) VALUES ($1, $2, $3) RETURNING *";
...
const getAuthorById = async (req, res) => {
  ...
  try {
    const { rows } = await pg.query("SELECT * FROM authors WHERE id = $1", [
      id,
    ]);
    if (rows.length === 0) {
      return res
...
const updateAuthor = async (req, res) => {
  ...
  if (!id || isNaN(id)) {
    return res
      .status(400)
      .json({ success: false, message: "Invalid author ID: must be a number" });
  }
  ...
  try {
    ...
    const value = [name, gender, profile, authorId];
...
const deleteAuthor = async (req, res) => {
  ...
  try {
    ...
    if (error.code === "23503") {
      // Foreign key violation (author linked to books)
      return res
        .status(409)
        .json({
          success: false,
